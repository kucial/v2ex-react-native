import { MemberBasic, NodeBasic, TopicBasic, TopicReply } from "@/types/v2ex";
...
export function topicDetailFromPage($: CheerioAPI, id: string | number) {
  const member = memberFromImage($('#Wrapper .header a[href^="/member"] img').first());
  const node = nodeFromLink($('#Wrapper .header a[href^="/go"]').first());
  const title = $('#Wrapper .header h1').text().trim();
  const content_rendered = $('#Wrapper .cell .topic_content').html()?.trim() || '';


  let replies = 0;
  let last_reply_time;
  if ($('.cell[id^=r_]').length) {
    const compos = $('.cell[id^=r_]').parent().children().first().text().trim().split('•');
  ...
  const subtles = $('#Wrapper .content .subtle').map(function (i, el) {
    return {
...
export function topicReplyFromCell($el: Cheerio<Element>, $: CheerioAPI): TopicReply {
  const member = memberFromImage($el.find('img.avatar').first());
  const content_rendered = $el.find('.reply_content').html();
  const members_mentioned = $el.find('.reply_content a[href^="/member/"]').map(function (j, a) {
    return $(a).text().trim()
  }).get();
  ...
  const replyInfo = $el.find('td:nth-child(3) span.fade.small').first().text().trim();
  let reply_time;
  let reply_device;
  [reply_time, reply_device] = replyInfo.split(' via ');
  const heartImg = $el.find('td:nth-child(3) span.small.fade img[alt="❤️"]').first();
