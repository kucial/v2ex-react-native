...
import { BASE_URL, REQUEST_TIMEOUT, USER_AGENT, ONCP } from './constants'
import { StatusResponse, EntityResponse, PaginatedResponse, CollectionResponse, TopicId, ReplyId } from './types'
import { memberFromImage, topicFromLink, nodeFromLink, paginationFromText, topicDetailFromPage, topicReplyFromCell, userMetaForCurrentUser, nodeDetailFromPage } from './helpers';
...
instance.interceptors.response.use(function (res) {
  ...
  if (res.config?.url && /^\/(\?tab=\w+)?$/.test(res.config.url)) {
    const $ = load(res.data);
    let text = $('input.special.super.button').attr('value');
    let unread_count = 0;
    if (text) {
...
export async function getHomeTabs(): Promise<CollectionResponse<HomeTabOption>> {
  const { data: html } = await request({
    url: '/',
  });
  const $ = load(html);
  const tabs = $('#Wrapper .content a[class^=tab]').map(function (i, el) {
    return {
      label: $(el).text().trim(),
...
export async function getHomeFeeds(
  ...
  const $ = load(html);
  const data = $('#Wrapper .content .cell.item').map(function (i, el) {
    ...
    const last_reply_by = $(el).find('td:nth-child(3) span:last-child a').text();
    const last_reply_time = $(el).find('td:nth-child(3) span:last-child').text()?.split('•')[0].trim();
    return {
      ...topic,
      last_reply_by,
      last_reply_time,
...
export async function getRecentFeeds(
  ...
  const paginationText = $('#Wrapper .content .inner:last-child [align=center]').text();
...
export async function reportTopic(
  ...
  const $ = load(html);
  const reported = $('span.fade').get().some(function (el) {
...
export type TopicRepliesResponse = PaginatedResponse<TopicReply, { topic: TopicDetail }>
export async function getTopicReplies(
  { id, p = 1 }: { id: number, p?: number }
  ...
  const data = $('#Wrapper .content .box div[id^=r_]').map(function (i, el) {
...
export async function postReply(
  { id, content, once = ONCP }: PostReplyPayload
): Promise<StatusResponse<TopicReply>> {
  let postOnce = once;
  if (postOnce === ONCP) {
    const onceRes = await fetchOnce();
    postOnce = onceRes.data
  ...
  const topic = topicReplyFromCell($('#Wrapper .content .box div[id^=r_]').last(), $);
  return {
    success: true,
    message: '回复成功',
    data: topic
...
export async function thankReply(
  { id, once = ONCP }: { id: ReplyId, once?: string }
...
export async function getNodeGroups(): Promise<CollectionResponse<NodeGroup>> {
  ...
  const $ = load(html);
  const groups = $('#Wrapper .content > .box').map(function (i, el) {
    ...
    const nodes = $(el).find('.inner a.item_node').map(function (j, n) {
...
export async function collectNode(
  { name, once = ONCP }: { name: string, once?: string }
...
export async function getNodeFeeds(
  ...
  const data = $('#Wrapper .content > .box:nth-child(2) .cell').map(function (i, el) {
...
export async function watchMember(
  { id, username, once = ONCP }: { id?: number, username?: string, once?: string }
...
export async function getMemberTopics(
  { username, p = 1 }: { username: string, p: number }
  ...
  const $ = load(html);
  if ($('#Wrapper .content .box .cell').length == 1 && $('#Wrapper .content .box .cell img').attr('src')?.indexOf('lock') > -1) {
    throw new ApiError({
      code: 'member_locked',
  ...
  const data = $('#Wrapper .content .box .cell').map(function (i, el) {
    if (!$(el).find('table').length) {
      return null;
    }
    const member = {
      username: $(el).find('td:nth-child(1) strong a').first().attr('href').replace('/member/', ''),
      avatar_normal: undefined,
    }
    const node = nodeFromLink($(el).find('td:nth-child(1) .node'));
    const topic = topicFromLink($(el).find('.topic-link'));
    const last_reply_time = $(el).find('td:nth-child(1) span:last-child').text().split('•')[0].trim();
    const last_reply_by = $(el).find('td:nth-child(1) span:last-child a').text();
...
export async function getMemberReplies(
  ...
  const data = $('#Wrapper .content .box .dock_area').map(function (i, el) {
...
export async function getMyNotifications(
  ...
  const $ = load(html)
  const notifications = $('#notifications .cell').map(function (i, el) {
...
export async function getMyCollectedTopics(
  ...
  const $ = load(html);
  const data = $('#Wrapper .box .cell.item').map(function (i, el) {
    const member = memberFromImage($(el).find('td:nth-child(1) img'));
    if (!member.username) {
      member.username = $(el).find('td:nth-child(1) a').attr('href').replace('/member/', '');
    }
    const topic = topicFromLink($(el).find('.item_title a'));
    const node = nodeFromLink($(el).find('td:nth-child(3) a.node'));
    const last_reply_time = $(el).find('.topic_info span[title]').text().trim();
    const last_reply_by = $(el).find('.topic_info strong:last-child a').text().trim();
...
export async function getMyCollectedNodes(): Promise<CollectionResponse<NodeExtra>> {
  const { data: html } = await request({
    url: '/my/nodes',
  })

  const $ = load(html);
  const data = $('#my-nodes .fav-node').map(function (i, el) {
...
export async function getCurrentUser(): Promise<EntityResponse<MemberDetail, { unread_count: number }>> {
  const { data: html } = await request({
    url: '/'
  });
  const $ = load(html);
...
export async function dailySignin(data: { once?: string } = { once: ONCP }): Promise<StatusResponse> {
  const { data: html } = await request({
...
export async function logout({ once = ONCP }: { once?: string }): Promise<StatusResponse> {
  const { data: html } = await request({
    url: `/signout`,
    params: {
      once,
