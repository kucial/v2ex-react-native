...
export function getEmojiTextFromImgurUrl(url: string): string | undefined {
  if (!url) return undefined
  return IMGUR_TO_EMOJI_TEXT_MAP.get(url)
}

export function getTrueEmojiFromImgurUrl(url: string): string | undefined {
  if (!url) return undefined
...
export function replaceImgurImagesWithTrueEmoji(html: string): string {
  ...
  // 1. First replace any <a> tag wrapping an Imgur emoji <img> or just the <img> tag pointing to an Imgur emoji
  let result = html.replace(
    /(?:<a[^>]*href=["']https?:\/\/(?:i\.)?imgur\.com\/[^"']+["'][^>]*>\s*)?<img[^>]*src=["'](https?:\/\/(?:i\.)?imgur\.com\/[^"']+)["'][^>]*>(?:\s*<\/a>)?/gi,
    (match, imgUrl) => {
      const trueEmoji = IMGUR_TO_TRUE_EMOJI_MAP.get(imgUrl)
      if (trueEmoji) {
        return trueEmoji
      }
      return match
  ...
  // 2. Replace text emoji placeholders like [doge] with the true emoji character if found in HTML text
  result = result.replace(/\[[^\]]+\]/g, (match) => {
    const trueEmoji = EMOJI_TEXT_TO_TRUE_EMOJI_MAP.get(match)
    if (trueEmoji) {
      return trueEmoji
    }
    return match
