...
import useSWRBase, { Cache, SWRResponse } from 'swr'
...
export const isRefreshing = (swrState: SWRInfiniteResponse | SWRResponse) => {
  ...
  if ('size' in swrState) {
    return !!(swrState.isValidating && swrState.data && swrState.data.length === swrState.size);
  }
  return !!(
...
export const isInitState = (swr: SWRResponse) => swr.data === undefined && swr.error === undefined
...
export const isEmptyList = function isEmptyList<D = DefaultListData>(swr: SWRInfiniteResponse<D>, isEmptyCollection = DEFAULT_EMPTY) {
  return Array.isArray(swr.data) && swr.data.every((value) => {
